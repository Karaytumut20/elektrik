  ...
  return (
    <AdminShell>
      ...
      <div className="admin-mobile-list">
        {result.data.map((staff) => <article className="admin-mobile-item" key={staff.id}><h2 className="font-bold text-slate-950">{staff.full_name}</h2><dl className="admin-mobile-metrics"><div><dt>Görev</dt><dd>{staff.title ?? "—"}</dd></div><div><dt>Telefon</dt><dd>{staff.phone ?? "—"}</dd></div><div className="col-span-2"><dt>Çalışma saatleri</dt><dd>{staff.work_start.slice(0,5)} – {staff.work_end.slice(0,5)}</dd></div></dl></article>)}
        {result.data.length === 0 ? <div className="admin-mobile-item text-sm text-slate-500">{result.error ? "Personel tablosu henüz kurulmamış olabilir." : "Personel kaydı yok."}</div> : null}
      </div>
      <div className="hidden sm:block"><div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Ad soyad</th><th>Görev</th><th>Telefon</th><th>Çalışma saatleri</th></tr></thead><tbody>
